...
1. **Per-hop traceroute / MTR** — for the **gateway/internet (WAN) path** only: the ProjectorPC↔ryzen LAN path is Layer-2 switched (coupler + Tenda switches don't decrement TTL), so traceroute can't isolate it — NIC change-events cover the L2 side instead. Build traceroute when a WAN-side fault is suspected.
