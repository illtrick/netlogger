...
## What this is
A **portable, self-elevating native desktop app** (Go + Gio) that diagnoses intermittent, load-triggered LAN problems by running the same `NetLogger.exe` on every machine. Instances **auto-discover** each other on the LAN and continuously **measure every link** (ICMP + high-rate UDP with jitter/micro-drop capture) plus the **gateway** — to isolate whether a fault is an endpoint NIC, a shared switch, or the router. Original symptom: Moonlight stutter Desktop-A→HTPC. No web UI, no installer, no services, no terminal — double-click, approve one UAC prompt, read the window.

## Status (milestones N1–N3e + matrix + UX + diagnostics + NIC complete; legacy retired; all on `main`)
- **N1** portable elevated Gio app shell + lifecycle. **N3a** identity + LAN multicast discovery. **N3b** discovery wired in (peers visible). **N3c** per-peer ICMP RTT/loss. **N3d** high-fidelity 200 Hz UDP probing (jitter + micro-drop episodes) + multi-homed primary-IP fix. **N3e** default-gateway **and internet** ICMP probing. Each verified live on desktop-a + HTPC.
- **Link Matrix** (N×N source→dest loss grid, CVD-safe bands, gossiped link stats). **UX overhaul** (header status chip + uptime, infra/peer sparklines, matrix + legend, compact footer). **Prevent-sleep** (Win32 `SetThreadExecutionState` keep-awake, UI toggle). **Mesh reset** (`/api/command` ResetAll — one click restarts the logging session on every machine at once) + **Export** bundle (one-click analysis JSON beside the exe). **Diagnostics persistence** (UDP iso drops + connectivity events now written to the store). **NIC diagnostics** (per-adapter link speed/status, **all power-saving properties** (EEE / Green Ethernet / Gigabit Lite / Ultra Low Power / …, not just first match), error/discard counters with per-poll delta; UI Adapters section; in the export bundle).
- **Build stamp + skew detection** (`version.Build` from git hash via ldflags; `LinkReport` carries it; UI shows a vermillion banner when a peer runs a mismatched exe; footer shows this machine's build). **Truthful mesh reset** (ResetAll counts real per-peer acks; a non-acking peer is reported, not hidden behind an optimistic message). **NIC change events** (link down/up, speed renegotiation 2.5G→1G, edge-triggered discard/error episodes recorded on the connectivity timeline — the Layer-2 evidence traceroute can't see, since the LAN path is L2-switched). **Mesh-wide live event log** (`/api/events` endpoint + pull in `linkPullLoop`; `Snapshot.Events` merges this machine's + every peer's events into one time-sorted, host-labeled timeline → "Recent events (mesh-wide)" UI panel, newest first, offline events vermillion — so a cross-machine fault is legible on one screen without dual exports; cross-machine ordering assumes NTP-synced clocks, a clock-offset handshake is future work).
...
Modem(Calix C5500XK)→Router(TP-Link BE9300)→Switch1(Tenda TEM2010F 2.5G)→{NAS(QNAP TS-563); HTPC(I219-V 1G via UGREEN coupler); Switch2(Tenda)→Desktop-A(Killer E3100G 2.5G **+ AX1675x Wi-Fi** — multi-homed!), Desktop-B(Intel I226-V 2.5G)}. **LAN is 2.5G end-to-end.** Leading hardware suspects: I226-V/E3100G **EEE dropout** > Switch1 > coupler.

## Diagnostic findings so far (live, on desktop-a + HTPC)
- The wired desktop-a↔HTPC link measures **clean** (sub-ms jitter, ~0 drops) at idle and under a 4 GB file write — i.e. bulk TCP doesn't reproduce the fault, and the **direct wired link is not the cause** of the disconnect observed.
- During a real **RDP disconnect** (user remotes in from a third LAN machine), the desktop-a↔HTPC probe stayed clean → the fault is on a path **not between those two** (the RDP-source machine's link, Wi-Fi, or the shared router). Hence N3e (gateway probing) and the guidance to **run NetLogger on the RDP-source machine too**.
- **DB analysis (HTPC, 38k samples / 7h45m):** real intermittent ICMP loss HTPC→desktop-a **1.36%** (vs only 0.37% to laptop-pc), RTT healthy when not lost (total micro-drops, not latency). Smoking-gun episode 20:11–20:40 sustained 5–30%/min with both apps running steadily. Points at **desktop-a's NIC (EEE-dropout lead)**, not a shared path — which is exactly what the new **NIC diagnostics (EEE state + discard deltas)** is built to confirm/kill. Caveat: at capture time UDP iso drops were **not** persisted (now fixed) — the next overnight export will carry them.
- desktop-a is multi-homed (wired `.154` + Wi-Fi `.24`); for a clean wired diagnosis, disable Wi-Fi on desktop-a during testing.

## Next steps (not yet built)
1. **Per-hop traceroute / MTR** — for the **gateway/internet (WAN) path** only: the HTPC↔desktop-a LAN path is Layer-2 switched (coupler + Tenda switches don't decrement TTL), so traceroute can't isolate it — NIC change-events cover the L2 side instead. Build traceroute when a WAN-side fault is suspected.
...
3. **Catch the fault in the act:** run NetLogger on all 3+ machines (incl. the RDP-source PC), reproduce the disconnect/stutter, read which row spikes (gateway = router; one peer = that NIC/cable; all-from-one-box = that NIC) and whether desktop-a's **Adapters** row shows EEE-enabled / rising discards.
